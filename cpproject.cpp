...
void searchAndIssueBook(const string& username) {
    ...
    cout << yellow << "\n\n\t\t\t\t\tEnter the name of the book you want to search and issue: " << reset;
    cin.ignore();
    getline(cin, bookName);
    cout << yellow << "\n\t\t\t\t\tEnter the author's name: " << reset;
    getline(cin, author);
    ...
    for (auto& book : books) {
        if (book.bookName == bookName && book.author == author) {
            if (book.stock > 0) {
                ...
                cout << "Do you want to issue book \"" << bookName << "\" by " << author << "?" << endl;
...
void saveUserData(const Admin& user, const string& filename) {
    ...
    outFile << user.name << ',' << user.id << ',' << user.bookIssued << endl;
    outFile.close();
}
...
void displayUserData(const string& filename) {
    ...
    while (getline(inFile, line)) {
        ...
        ss >> user.id;
        ss.ignore();
        getline(ss, user.bookIssued, ',');
        cout << "| " << left << setw(20) << user.name << " | "
            << setw(5) << user.id << " | "
            << setw(20) << user.bookIssued << " |" << endl;
    }
    cout << "--------------------------------------------------------------------------------" << endl;
    inFile.close();
}
